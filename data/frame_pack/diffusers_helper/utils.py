...
def save_bcthw_as_mp4(x, output_filename, fps=10, crf=0):
    ...
    # 确保输入是3通道（RGB）或4通道（RGBA）
    if c not in [3, 4]:
        # 如果是1通道（灰度），复制成3通道
        if c == 1:
            x = x.repeat(1, 3, 1, 1, 1)
        else:
            raise ValueError(f"不支持 {c} 通道的视频输入，应为1、3或4通道")
    ...
    # 重排张量为网格布局: [t, (rows*h), (cols*w), c]
    x = einops.rearrange(x, '(m n) c t h w -> t (m h) (n w) c', n=per_row)

    # 转换为 numpy 数组 (imageio 需要)
    video_frames = x.numpy()

    # 确保颜色通道顺序正确 (RGB 而不是 BGR)
    if video_frames.shape[-1] == 3:
        # 如果颜色不正常，可以尝试交换通道顺序
        video_frames = video_frames[..., [2, 1, 0]]  # BGR 转 RGB
        pass
    ...
    cmd = [
        ...
        '-pix_fmt', 'yuv420p',
        '-color_range', '2',  # 全范围颜色
        '-vf', 'scale=out_color_matrix=bt709',  # 明确颜色矩阵
        output_filename
    ]
