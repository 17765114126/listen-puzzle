...
from db.Do import BaseReq, we_library, ChatHistory
...
async def chat_del(id: int):
    # 删除记录列表
    # we_library.fetch_all("del * FROM chat_history", tuple([]))
    return True
...
def chat(req: BaseReq):
    ...
    # 添加新消息
    content.append(prompt_messages)
    content.append({"role": "assistant", "content": resp})
    chat_history.content = json.dumps(content)  # 序列化为字符串
